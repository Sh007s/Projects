...
void display(int adc_reg_val_speed)
{
    ...
    do
    {
        buff[i] = (adc_reg_val_speed % 10) + '0';
        adc_reg_val_speed = adc_reg_val_speed / 10;
    } while (i--);
    buff[2] = '\0';
    clcd_print(buff, (0xC0 + 14));
}
...
int check_password()
{
    ...
    clcd_print("                ", (0xC0 + 0));
    unsigned char temp_password = 0, key_state = 0xFF, count = 0;
    while(1)
    {
        key_state = read_matrix_keypad(1);
...
void main(void) {
    ...
    while (1) {
        clcd_print("Time      G   SP", (0x80 + 0));
        key_state = read_matrix_keypad(1);
        key_level = read_matrix_keypad(0);

        unsigned int adc_reg_val_speed;
        adc_reg_val_speed = (read_adc(0x00) / (float)1024) * 100;
        ...
        if(menu_flag == 0)
        {
            ...
            if(key_state == 2)
            {
                ...
                clcd_print(gear[gear_idx], (0xC0 + 10));
        ...
        else
        {
            ...
            if(password_flag == 0 && check_password() != 1)
            {
                    ...
                    switch(menu_select)
                    {
                        ...
                        case 1:
                        {
                            if(time_count_isr >= 10000)
                            {
                                ...
                                clcd_print("   Change passwd", (0xC0 + 0));
                            }
                            else
                            {
                                clcd_print("-> Set time     ", (0x80 + 0));
                    ...
                    while(1)
                    {
                        ...
                        if(time_count_isr < 10000)
                        {
                            switch(field_select)
                            {
                                case 0:
                                {
                                   clcd_print("__",(0xC0 + 0));
                                   break;
                                }

                                case 1:
                                {
                                   clcd_print("__",(0xC0 + 3));
                                   break;
                                }

                                case 2:
                                {
                                   clcd_print("__",(0xC0 + 6));
                                   break;
                                }
                            }
                        }
                ...
                {
                    clcd_print("Entr prev Passwd", (0x80 + 0));
                    if(check_password() == 1)
                    {
